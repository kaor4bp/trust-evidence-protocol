#!/usr/bin/env python3
"""Shared helpers for repo-local Claude Code hook adapters."""
...
def _plugin_root_candidates() -> list[Path]:
    # CLAUDE_PLUGIN_ROOT is provided by Claude Code when running installed plugins
    env_root = os.environ.get("CLAUDE_PLUGIN_ROOT")
    candidates: list[Path] = []
    if env_root:
        candidates.append(Path(env_root))
    candidates.extend([PLUGIN_SOURCE_ROOT, REPO_ROOT / "plugins" / "trust-evidence-protocol"])
