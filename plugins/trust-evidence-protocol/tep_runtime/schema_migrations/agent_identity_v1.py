...
class AgentIdentityV1Migration:
    ...
    def migrate(self, path: Path, record: dict, *, relative_path: str) -> SchemaMigrationResult:
        ...
        if key_scope and key_scope != "agent-owned":
            issues.append(
                SchemaMigrationIssue(
                    path=relative_path,
                    reason="unsupported_agent_key_scope",
                    detail="AGENT-* record_version=1 requires key_scope=agent-owned.",
                    migration_id=self.id,
                    record_id=record_id,
                )
            )
        ...
        migrated.setdefault("key_scope", "agent-owned")
        migrated.setdefault("status", "active")
        if not str(migrated.get("note", "")).strip():
            migrated["note"] = "Migrated AGENT identity metadata. Private key material remains agent-held."
