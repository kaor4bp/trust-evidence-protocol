...
- `chain_permits.ttl_seconds` for `GRANT-*` lifetime
...
- `probe_route`: ordered generated inspection route for one curiosity probe, including next commands plus diagram/pack expansion hints
...
- Use `probe_route` when you want the runtime to compose the next inspection commands for a selected probe, including diagram/full-pack expansion hints; route output is navigation only, not proof.
...
- `configure-runtime`
  ...
  - shows or updates `chain_permits.ttl_seconds` grant TTL
...
- `lookup --query "..." --reason orientation|planning|answering|permission|editing|debugging|retrospective|curiosity|migration --kind facts|code|theory|research|policy|auto [--format text|json]`
  ...
  - owner-bound mutations require a private per-agent key: pass `agent_private_key` in MCP calls, or set `TEP_AGENT_PRIVATE_KEY` for CLI/hooks; agents must invent their own key and not share another agent's key
...
- `record-link --left CLM-* --right CLM-* --relation related --quote "..." --note "..."`
  ...
  - appears in `map_graph` as a generated `related` edge between the reviewed support claims
...
- `input-triage`
  - `report` lists unclassified `INP-*` records, optionally by task
  - `link-operational --task TASK-*` links operational prompts to a generated `WCTX-*` via `input_refs`
  - use this for prompts such as "continue", "reloaded Codex", or other non-semantic workflow signals that should not become `SRC/CLM/GLD`
